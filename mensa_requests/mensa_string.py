...
ign_names = ['T?glich wechselndes Dessertangebot.', 'T?glich wechselnde Dessertvariationen.',
             ...
             'Pommes-Spezialit?t']
ign_sides = ['Beilagendessert', 'Beilagensalate','Eintopf']
...
def parse_menu_week(tree):
    ...
    menu_dates = [d.text.strip() for d in menu_dates]
    mensa_menus_week = list()
    for index, day in enumerate(menu_week):
        day_plan = dict()
        # typical date string: "Montag, 04.04.2022", also possible: "Wochenangebote KW 14"
        try:
            day_date = menu_dates[index].split(' ')[1]
            day_plan['date'] = datetime.datetime.strptime(day_date, '%d.%m.%Y').date()
        except:
            day_plan['date'] = menu_dates[index]
        ...
        for day_menu in day_menus:
            ...
            if (itm is not None) and (menu_type not in ['Mensa vital']):
                menu_sides = (''.join(itm.xpath("./text()"))).strip()
                ...
                if menu_sides in [['auch zum Mitnehmen!']]:
                    menu_sides = []
                menu_sides = [i for i in menu_sides if i not in ign_sides]
                if '' in menu_sides:
                    menu_sides = []
...
try:
    ...
    # print output
    order = [u'Menü 1', u'Salattheke', u'Tagesmenü', u'Menü vegetarisch', u'Eintopf', u'Eintopf / Suppe', u'Mensa vital', u'Mensa kulinarisch', u'Aktions-Theke']
    menus = list()
    menu_ind = None
    return_dict = []
